...
"""
We comment and study the original file of training SSD on VOC.
This file is comment in order to understand the main pipeline
of training SSD.

The main goal is to change the loss function in order to apply the Distillation
in our case. Moreover, we want to introduce the output of the teacher that we save
before. The temperature that we use for the soft target is equal to 5.

gluoncv has already implemented most of the loss function that we need. That's why, we only seek to 
insert the good loss function, and the good data so as to make the distillation framework work.

Secondly, we try to add the Hint Framework.
"""
...
def parse_args():
    """
    This function is needed to add option when we enter the command line on the
    shell in order to run the file.
    """
...
def get_dataset(dataset, args):
    """
    This function is primordial, we need to get the dataset. This function
    Argument :
    =======
    dataset : strg
    ...
    if dataset.lower() == 'voc':
        ...
        val_dataset = gdata.VOCDetection(
            splits=[(2007, 'test')])
        # the metric is needed to evaluate our network, it calculates the average precision for VOC
...
def get_dataloader(net, train_dataset, val_dataset, data_shape, batch_size, num_workers, ctx):
    ...
    In addition, to be able to run in mini-batches, images must be resized to the same shape.

    A handy DataLoader would be very convenient for us to apply different transforms
    and aggregate data into mini-batches.
    ...
    batchify.Pad handles padding automatically.
    batchify.Stack is used to stack NDArrays with consistent shapes.
    batchify.Tuple is used to handle different behaviors across multiple outputs from transform functions.
    (behavior of batchify_fn: stack images, and pad labels)

    Argument:
    ========
    net = network
    train_dataset, val_dataset = Cf et_dataset
    batch_size = integer
    num_workers = we can chose it larger than 0 (if your CPU has more cores) to accelerate data loading

    ctx = context use of GPU or not
    data_shape = 600, 1000 (for example)

    Return:
    ======
    train_loader, val_loader
    We can iterate on those output, in order to get the batch (input of our neural network) and work with it like for
    training.
    """
...
def save_params(net, best_map, current_map, epoch, save_interval, prefix):
    """
    This function is to save the weight of our neural network after the training.

    Return:
    ======
    No return, only the fact that the weights are saving.
...
def validate(net, val_data, ctx, eval_metric):
    ...
    """
    eval_metric.reset()
    # set nms threshold and top-k constraint
...
if __name__ == '__main__':
    ...
    if args.dali:
        """
        DALI Dataset, but not in our interest....
        """
        if not dali_found:
        ...
        """
        Voc Dataset, concerned of our project
        """
