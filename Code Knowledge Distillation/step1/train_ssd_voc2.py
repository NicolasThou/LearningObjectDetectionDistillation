...
print('Train label2:', train_label2)
print('train label2 shape', train_label2.shape)
...
print("The network has an architecture like : ")
print(net)
...
print()
print("=============== training output of the network ================")
print()
...
# cls_preds (1, 45598, 21), there are 21 classes
# boxe_preds (1, 45598, 4), there are 45598 bounding boxes with 4 offset predicted per anchors boxes
# anchors (1, 45598, 4), there are 45598 anchors boxes with 4 coordinates
...
# batch[0] (10, 3, 1000, 600) this is the data image input
# batch[1] (10, 45598) this is the class targets for each box target
# batch[2] (10, 45598, 4) this is the boxes target with 4 coordinates

##############################################################################
# Loss, Trainer and Training pipeline
print()
print("============ Loss, Trainer and Training pipeline =============")
print()
from gluoncv.loss import SSDMultiBoxLoss
from gluoncv.CustomLoss import *
# cp CustomLoss {Path to /lib/python3.7/site-packages/gluoncv}
# Example :
# cp CustomLoss /Users/nicolasthou/anaconda3/envs/deep_learning/lib/python3.7/site-packages/gluoncv

ssd_loss = SSDMultiBoxLoss()
...
for ib, batch in enumerate(train_loader):
    ...
    print('data:', batch[0].shape)  # dim [5, 3, 512, 512]
    print('class targets:', batch[1].shape)  # dim [5, 24656]
    print('class targets:', batch[1])  # for each bounding boxes, there is a class assigned. So 24656 Bounding Boxes with each : one classe
    print('box targets:', batch[2].shape)  # dim [5, 24656, 4]
    print('box targets:', batch[2])  # For each image (5), we have 24656 bounding boxes, with 4 coordinates

    with autograd.record():
        # Prediction - output of the network
        cls_pred, box_pred, anchors = net(batch[0])
        print('cls_pred:', cls_pred, cls_pred.shape)  # dim [5, 24656, 21]
        print('box_pred:', box_pred, box_pred.shape)  # dim [5, 24656, 4]
        print('anchors:', anchors, anchors.shape)  # dim [1, 24656, 4]
        # Compute the loss function
        sum_loss, cls_loss, box_loss = ssd_loss(
            cls_pred, box_pred, batch[1], batch[2])  # dim 5, dim 5, dim 5

        print()
        print("Display of loss")
        print(sum_loss, cls_loss, box_loss)
        print()
